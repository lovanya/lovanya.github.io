...
**生产案例**：大型 ERP 首页要加载 12 个 JS chunk（vendor、polyfill、4 个业务模块、路由、状态管理、监控、统计、SDK），HTTP/1.1 下需要 6 + 6 = 12 个 RTT（两批），**单次首屏 = 6 × RTT**。
...
**大型 ERP 升级到 Cloudflare + HTTP/3 后，TTFB 从 800ms 降到 80ms**（4.2s→1.8s 首屏优化的第一步）。

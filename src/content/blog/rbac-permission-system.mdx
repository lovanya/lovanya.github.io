...
1. **"SSO 统一权限项目"是真实经验**。权限系统是**企业级系统的大门**,没做好 = 任何业务模块都有安全风险。
...
**生产案例**：大型 ERP + 管理后台 + 财报系统 + 内部 IM 全部接同一个 SSO,**用户一次登录全公司通行**。
...
>
> **Q5(思考)**:你在 SSO 统一权限项目里是怎么设计的?多个子系统的权限怎么统一管理?
>
> **A**:**SSO 统一权限设计**:① **中央认证服务**— 单点登录,所有子系统跳转登录;② **统一用户中心**— 用户 / 角色 / 权限 集中管理,改一处生效全部;③ **OAuth 2.0 授权码模式**+ **JWT**——子系统拿 code 换 access_token;④ **权限分层**— 全局角色（平台级)+ 业务角色（子系统级),支持「一个用户在 ERP 是 admin,在财报系统是 user」;⑤ **审计**— 所有登录 / 权限变更记录到 ELK,**安全团队可查**。**效果**:10+ 子系统一次登录全通行,权限改动一处生效,**节省 80% 重复登录 + 权限管理成本**。<AIAsk question="你在 SSO 统一权限项目里是怎么设计的?多个子系统的权限怎么统一管理?" />

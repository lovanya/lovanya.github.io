...
1. **那个 12min→3min 构建优化是硬数据**。大型 ERP 重构用真实数字证明了工程价值——**不是聊性能，是把工具链配对**。
...
**痛点**：
- **冷启动慢**：大型 ERP 12min → 主要是 Webpack 冷启动 + 大量 Loader 解析
...
**生产案例**：大型 ERP 用这 4 个优化把构建从 12min 降到 4min——**但仍不够快**，才有了 Vite / Rspack 的尝试。
...
>
> **Q5（思考）**：你在大型 ERP 项目里是怎么把构建时间从 12min 优化到 3min 的？请说具体每一步的耗时拆解和对应优化。<AIAsk question="你在大型 ERP 项目里是怎么把构建时间从 12min 优化到 3min 的？请说具体每一步的耗时拆解和对应优化。" />

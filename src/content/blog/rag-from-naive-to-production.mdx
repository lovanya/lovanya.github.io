...
**修法**：见下文技法 1（query rewriting）。

### 坑 3：检索返回的 chunk 质量差

向量相似度高 ≠ 答案相关。两个文档话题相似但**方向相反**（"应该做 X" vs "不要做 X"），top-K 全召回反而误导 LLM。

**修法**：见下文技法 3（reranking）。

### 坑 4：缺乏 metadata 过滤

100 万条 chunks 里，60% 是过期内容。LLM 拿到旧答案照答不误。

**修法**：建索引时给每条 chunk 加 metadata（`created_at`, `category`, `source`），检索时**先 metadata 过滤再相似度排序**。
...
- **Prompt cache**：OpenAI / Anthropic 对重复 prefix 自动 cache，节省 90% 成本 + 延迟
...
100 QPS × 1000 tokens × 1M Q/月 = 100B tokens/月。

...
<Mermaid client:load caption="图 1：Prompt Cache 命中原理" chart={`flowchart LR
  R[请求] --> PC{前缀匹配?}
  PC -->|是, 1024+ tokens| Cache[命中缓存<br/>便宜 90%]
  PC -->|否| Miss[未命中<br/>全价]
  Cache --> LLM[LLM]
  Miss --> LLM
  LLM --> Out[Response]
...
**OpenAI** 自动缓存任何**>= 1024 tokens** 的 prompt 前缀，命中部分便宜 50%（写入多 50%，读取便宜 50%）。**Anthropic** 类似的 5 分钟 TTL cache 缓存**4 个 breakpoints**。
...
**关键**：稳定内容（system prompt + few-shot examples + 长期上下文）放最前面，**至少凑够 1024 tokens** 让 cache 生效。
...
1. ✅ **Prompt Cache 优化**：稳定内容放最前面，至少 1024 tokens
2. ✅ **全链路流式**：TTFT < 500ms
...
- **Prompt Cache 90% token 节省**：稳定内容放最前面凑够 1024 tokens。

...
[LangChain 推荐](https://docs.langchain.com/oss/python/langchain/tools) 用 `wrap_tool_call` 中间件统一捕获错误：
...
- 危险工具（转账、删除）默认 `toolChoice: 'user-approval'`，需要人工确认
...
**解法**：动态工具选择
- 平时只暴露 5-10 个核心工具
- 大型 / 冷门工具按需通过 `tool_search` 加载（仅 gpt-5.4+ 支持）
...
- **危险工具必须人工审批**：转账、删除、发邮件——Vercel AI SDK 用 `toolApproval: 'user-approval'`。

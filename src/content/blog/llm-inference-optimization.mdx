...
<Mermaid client:load caption="图 1：LLM 全生命周期成本" chart={`pie title LLM 全生命周期成本（典型企业）
  "推理 API 费用" : 75
  "训练 / 微调" : 10
  "工程 + 人力" : 12
  "数据 + 存储" : 3`} />
...
<Mermaid client:load caption="图 2：LLM 推理循环" chart={`flowchart LR
...
<Mermaid client:load caption="图 3：PagedAttention 分页映射" chart={`flowchart TB
...
<Mermaid client:load caption="图 4：量化方案选型" chart={`flowchart TD
  Q1{部署在哪里?}
  Q1 -->|GPU 服务器| Q2{质量优先?}
...
<Mermaid client:load caption="图 5：Speculative Decoding 时序" chart={`sequenceDiagram
...
<Mermaid client:load caption="图 6：生产部署选型" chart={`flowchart TD
  Q1{每天 token 量?}

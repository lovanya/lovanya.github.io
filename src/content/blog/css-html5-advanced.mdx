...
3. **性能优化的真正门槛**。`will-change`、`contain`、`content-visibility` 这些现代 CSS 性能属性不靠 JS 控制，靠 CSS 引擎判断。大型 ERP 12min→3min 构建优化里有一半时间在调样式作用域。
...
**生产用法**：长列表 / 大表格的每个卡片加 `contain: content`，让浏览器对每个卡片独立渲染优化。大型 ERP 800+ 页面里表格行加 `contain: layout` 后，**滚动掉帧率降低 60%**。
...
2. **`flex: 1` 不等于 `flex-grow: 1`**。`flex: 1` 是 `flex: 1 1 0%`，**基准宽度为 0**——这正是"按比例分配剩余空间"的核心设计。`flex-grow: 1` 配 `flex-basis: auto` 是"按内容比例"，通常并非期望效果。

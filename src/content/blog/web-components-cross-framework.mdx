...
1. **8 个跨 Vue/React 通用组件是稀缺经验**。跨框架组件库的核心就是 Web Components + 框架 wrapper,**理解了能设计独立于框架的组件**。
...
**"8 个跨 Vue/React 通用组件"**——核心架构是 **Web Components 抽象 + 框架 wrapper**：
...
graph LR
  ...
  C --> E[大型 ERP<br/>finui.example.com]
  D --> F[管理后台<br/>rouyu.example.com]
```
...
**下一篇**:前端监控体系——错误 / 性能 / 行为监控, + 两段监控沉淀实战。

## 5 道重点面试问题方向

> **Q1(答案)**:Web Components 三大技术是什么?为什么 Web Components 是跨框架组件库的核心抽象?
>
> **A**:三大技术:① **Custom Elements**— `customElements.define` 注册自定义 HTML 标签,Vue / React / Angular / Solid 都能用;② **Shadow DOM**— 组件内 DOM / 样式隔离,不会和外部 CSS 冲突;③ **HTML Templates + Slot**— 跨框架一致的内容分发机制。**为什么是跨框架核心抽象**:Web Components 是 **W3C 标准**,所有现代浏览器原生支持,**不被任何框架绑定**。**8 个跨 Vue/React 通用组件的核心架构就是**:Web Components 写核心逻辑 + Vue/React wrapper 做适配——改一次核心,两个框架同步,节省 60% 维护成本。<AIAsk question="Web Components 三大技术是什么?为什么 Web Components 是跨框架组件库的核心抽象?" />
>

...
function Message({ message }) {
  // message.parts contains multiple parts streamed back from LLM
  // can be text, tool-call, UI components, JSON data
  return message.parts.map((part) => {
    if (part.type === 'text') return <p>{part.text}</p>;
    if (part.type === 'tool-weather') return <WeatherCard data={part.result} />;
    if (part.type === 'tool-chart') return <Chart data={part.result} />;
  });
}
```

**LLM returns JSON schema; frontend renders the corresponding component**.
...
**Vercel AI SDK's useChat supports tool call UI**:

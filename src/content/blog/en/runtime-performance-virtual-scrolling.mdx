...
1. **The FinUI large spreadsheet optimization is real.**300 columns × 500 rows, **virtual scrolling before / after gap 800ms → 50ms (16×)**.
...
**large ERP FinSpread table**: 300 columns × 500 rows, native `<table>` renders **800ms**, with virtual scroll **50ms (16× speedup)**.
...
**Next post**: Electron practice — main process / renderer process / USB Bridge / USB-hardware desktop app project practical experience.

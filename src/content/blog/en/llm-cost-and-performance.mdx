...
<Mermaid client:load caption="Figure 1: Prompt cache hit mechanics" chart={`flowchart LR
  ...
  PC -->|Yes, 1024+ tokens| Cache[Hit cache<br/>90% cheaper]
...
**OpenAI** auto-caches any prompt prefix **>= 1024 tokens**, charging 50% less for cached reads. **Anthropic** has a similar 5-min TTL cache with **4 breakpoints**.
...
**Key**: stable content (system prompt + few-shot examples + long-term context) goes first, **pad to at least 1024 tokens** so cache activates.
...
1. ✅ **Prompt cache optimization**: stable content first, pad to 1024 tokens
...
- **Prompt cache 90% token savings**: stable content first, pad to 1024 tokens.

...
<Mermaid client:load caption="Figure 1: Git Flow branching model" chart={`flowchart TB
  subgraph mainBranch["main (production)"]
    M[main]
  end
  subgraph devBranch["develop (integration)"]
    D[develop]
  end
  F1[feature/login] --> D
  F2[feature/payment] --> D
  D --> R[release/v1.0]
  R --> M
  R --> D
  M --> H[hotfix/bug-123]

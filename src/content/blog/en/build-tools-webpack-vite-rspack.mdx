...
1. **The 12min→3min build optimization  is hard data.**large ERP refactor with real numbers proves engineering value — **not talking about performance, but pairing the right toolchain**.
...
**Pain points**:
- **Slow cold start**: large ERP 12min → mainly Webpack cold start + many Loader parsing
...
**Production case**: large ERP used these 4 optimizations to take builds from 12min to 4min — **but still not fast enough**, which led to Vite / Rspack attempts.
...
>
> **Q5 (Think)**: In the large ERP project, how did you optimize build time from 12min to 3min? Walk through each step's time breakdown and matching optimization.<AIAsk question="In the large ERP project, how did you optimize build time from 12min to 3min? Walk through each step's time breakdown and matching optimization." lang="en" />
